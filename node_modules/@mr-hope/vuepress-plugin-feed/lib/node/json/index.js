...
const formatAuthor = (author) => (Object.assign(Object.assign({ name: author.name }, (author.url ? { url: author.url } : {})), (author.avatar ? { avatar: author.avatar } : {})));
/**
 * JSON 1.1 feed
 *
...
const renderJSON = (feed) => {
    ...
    if ((_a = channel.author) === null || _a === void 0 ? void 0 : _a.name) {
        content.author = Object.assign(Object.assign({ name: channel.author.name }, (channel.author.url ? { url: channel.author.url } : {})), (channel.author.avatar ? { avatar: channel.author.avatar } : {}));
    }
    content.items = feed.items.map((item) => {
        const feedItem = Object.assign(Object.assign({ title: item.title, url: item.link, id: item.guid || item.link }, (item.description ? { summary: item.description } : {})), { 
        ...
        if (Array.isArray(item.category))
            ...
            feedItem.tags = [item.category.name];
        return feedItem;
    });
