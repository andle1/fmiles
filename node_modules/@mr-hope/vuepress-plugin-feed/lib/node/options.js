...
const checkOptions = (options) => {
    ...
    if (hostname)
        ...
        (0, utils_1.error)("Option 'hostname' is required!");
        return false;
    }
    return true;
};
...
const getOutput = (output) => {
    ...
    return (0, vuepress_shared_1.deepAssign)(defaultOption, output || {});
};
...
const getFeedChannelOption = (options, context) => {
    ...
    const defaultChannelOpion = Object.assign(Object.assign(Object.assign({ title, link: (0, utils_1.resolveUrl)(hostname, base), description, language: (0, vuepress_shared_1.getRootLang)(context), copyright: author ? `Copyright by ${author}` : "", pubDate: new Date(), lastUpdated: new Date() }, (icon ? { icon } : {})), (image ? { image } : {})), (author ? { author: { name: author } } : {}));
...
const getFeedLinks = (options, output, context) => {
    ...
    return {
        atom: (0, utils_1.resolveUrl)(hostname, base, output.atom.path),
        json: (0, utils_1.resolveUrl)(hostname, base, output.json.path),
        rss: (0, utils_1.resolveUrl)(hostname, base, output.rss.path),
    };
};
