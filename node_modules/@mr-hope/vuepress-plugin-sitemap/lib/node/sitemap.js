...
const generatePageMap = (siteData, base, options) => {
    const { changefreq = "daily", exclude = ["/404.html"], dateFormatter = (page) => page.updateTimeStamp ? new Date(page.updateTimeStamp).toISOString() : "", } = options;
...
const genSiteMap = async (options, context) => {
    ...
    const { hostname, urls = [], outFile = "sitemap.xml", exclude = [], xmlNameSpace: xmlns, } = options;
    const sitemap = new sitemap_1.SitemapStream({
        hostname,
        xmlns,
    });
    const sitemapXMLPath = (0, path_1.resolve)(context.outDir, outFile);
    ...
    sitemap.end();
    console.log((0, chalk_1.blue)("Sitemap:"), chalk_1.black.bgGreen("Success"), `Sitemap generated and saved to ${(0, chalk_1.cyan)((0, path_1.relative)(context.cwd, sitemapXMLPath))}`);
    const robotTxtPath = (0, path_1.resolve)(context.outDir, "robots.txt");
    const robotsTxt = (0, fs_extra_1.existsSync)(robotTxtPath)
        ? await (0, fs_extra_1.readFile)(robotTxtPath, { encoding: "utf8" })
        : "";
